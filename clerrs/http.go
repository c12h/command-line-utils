...
// from the server or an error.  Callers can specify requests header and
// cookies, but cannot supply a request body.
//
// If the second parameter is not an empty string, it is used (without quotes) in
// error messages instead of the (quoted) URL string.
//
...
//
// MetaViaHTTP performs a HTTP "HEAD" request and returns the body of the
// response from the server or an error.  Callers can specify requests header
// and cookies, but cannot supply a request body.
//
// If the second parameter is not an empty string, it is used (without quotes) in
// error messages instead of the (quoted) URL string.
//
